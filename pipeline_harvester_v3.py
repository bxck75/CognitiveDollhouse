"""
PipelineHarvester v3: Fast pipeline morphing with RealESRGAN upscaling.
Loads from local safetensors, caches components, morphs between t2i/i2i/inpaint.
Now includes optional 2x upscaling before generation to combat quality loss.
"""
...
class RealESRGANUpscaler:
    ...
    def _load_model(self):
        ...
        try:
            # Try importing the proper RealESRGAN module
            try:
            ...
            # Initialize RRDBNet (23 blocks, 32 channels - standard x2 config)
            self.model = RRDBNet(
                num_in_ch=3,
                num_out_ch=3,
                num_feat=64,
...
class PipelineHarvester:
    """
    Fast pipeline morphing for t2i, i2i, inpaint with LCM LoRA + RealESRGAN upscaling.
    ...
    def generate_i2i(
        ...
        # Pre-upscale if enabled to preserve resolution
    ...
    def generate_inpaint(
        ...
        # Pre-upscale if enabled
        original_size = image.size
        if upscale_first and self.upscaler:
            logger.info(f"Upscaling {original_size} → 2x before inpaint")
            image = self.upscaler.upscale(image)
            mask = mask.resize(image.size, Image.NEAREST)  # Match new size
