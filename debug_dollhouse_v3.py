"""
Dollhouse Frontend v2: Higher quality conversion + RealESRGAN integration.
Fixed PIL↔Pygame conversions to avoid transposition bugs.
"""
...
def pil_to_pygame(pil_image: Image.Image) -> pygame.Surface:
    """Convert PIL Image to pygame Surface correctly"""
...
def pygame_to_pil(pygame_surface: pygame.Surface) -> Image.Image:
    """Convert pygame Surface to PIL Image correctly"""
...
def pygame_mask_to_pil(pygame_surface: pygame.Surface) -> Image.Image:
    """Convert pygame mask Surface to PIL grayscale"""
    arr = pygame.surfarray.array3d(pygame_surface)
    gray = arr[..., 0]
...
class Room:
    ...
    def apply_new_image(self, pil_image: Image.Image):
        """Apply new PIL image and start fade"""
    ...
    def reset_to_empty(self):
        """Reset room to original empty template"""
...
class DollhouseApp:
    def __init__(self):
        ...
        pygame.display.set_caption("Dollhouse — Live Proto v2")
    ...
    def _init_harvester(self):
        """Initialize pipeline harvester with RealESRGAN"""
        try:
            from pipeline_harvester_v3 import PipelineHarvester
    ...
    def draw(self):
        ...
        info = [
            ...
            f"ENTER:inpaint | BACKSPACE:img2img | Z:undo | R:reset | S:save | ESC:quit",
        ]
